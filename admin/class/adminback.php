...
class  adminback
{
    ...
    function update_logo($data) {
        ...
        if ($lg_ext == "jpg" || $lg_ext == 'jpeg' || $lg_ext == "png") {
            if ($lg_size <= 2e+6) {
                // Actualiza las siguientes líneas para limitar el tamaño a 64x64 píxeles
                if ($width <= 64 && $height <= 64) {
                    ...
                    $msg = "Sorry!! Logo max size should be 64x64 pixels, but you are trying {$width} px and {$height} px";
                    return $msg;
                }
            } else {
